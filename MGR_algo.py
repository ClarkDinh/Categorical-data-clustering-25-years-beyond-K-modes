...
https://www.sciencedirect.com/science/article/pii/S0950705114001026
https://github.com/cwyoon96/MGR-implementation
'''
import numpy as np
import copy
...
def mgr(data, k):
    ...
    ----------
    data: pandas DataFrame with datatype str
        data that needs to be clustered
    k: integer
    ...
    """
    # Initialize cluster label array
    ...
    while ((cnc < k) and (C != [])):
        ...
        labels[cluster_index] = cnc - 1
        
        # print(f'{cnc}th cluster assigned')
        
        C = list(set(C) - set(cluster_index))
        cnc += 1
        
        if ((cnc == k) and (C != [])):
            # Assign remaining data points to the last cluster
            labels[C] = cnc - 1
            break
